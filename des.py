...
def login():
    if request.method == 'POST':
        ...
        senha = hashlib.sha512(str(request.form.get('senha')).encode(
            "utf-8")).hexdigest()  # não é possível comparar a senha com
...
def novousuario():
    # transforma a senha em um hash com 128 caracteres,
    hash = hashlib.sha512(
        str(request.form.get('senha')).encode("utf-8")).hexdigest()

    usuario = Usuario(request.form.get('nome'), request.form.get('email'), hash, request.form.get('senha'), request.form.get('rua'), request.form.get('cidade'), request.form.get('estado'), request.form.get('cep'))  # em vez de utilizar "request.form.get('passwd')"
    # que guarda diretamente a senha no bancol se utiliza "hash,request.form.get('passwd')"  # que faz a conversão da senha em um hash
    db.session.commit()
...
def editarusuario(id):
    ...
    if request.method == 'POST':
        ...
        usuario.senha = hashlib.sha512(str(request.form.get('senha')).encode(
            "utf-8")).hexdigest()  # no alterar transformar em hash mas isso pode dar problema
